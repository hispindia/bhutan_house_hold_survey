...
export const TABLE_FIELDS =
  "++id, fileName, importStatus, uids, createdAt, updatedAt";
...
export const createImportFile = async ({
  ...
  try {
    ...
    console.log(
      `Import file record created with ID: ${id} for file: ${fileName}, UIDs: ${
        uids ? uids.split(",").length : 0
      } total`
    );
...
export const updateImportFileStatus = async (id, importStatus) => {
  try {
    ...
    if (!Object.values(IMPORT_STATUS).includes(importStatus)) {
      throw new Error(
        `Invalid import status: ${importStatus}. Must be one of: ${Object.values(
          IMPORT_STATUS
        ).join(", ")}`
      );
    }
...
export const findByDateRange = async (startDate, endDate) => {
  try {
    return await db[TABLE_NAME].where("createdAt")
...
export const bulkCreateImportFiles = async (importFiles) => {
  try {
    ...
    const ids = await db[TABLE_NAME].bulkAdd(recordsToCreate, {
      allKeys: true,
    });
