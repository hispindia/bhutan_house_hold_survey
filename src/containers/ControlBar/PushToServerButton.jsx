        ...
        for (const errorReport of error.validationReport.errorReports) {
          ...
          if (metadataMapping) {
            ...
            message = message.replace(uuidRegex, (match) => {
              ...
              return metadataMapping[match]
                ? `<b>${metadataMapping[match]}</b> (${match})`
                : `<b>${match}</b>`;
            });
          }
...
const handlePushToServer = async (
  ...
  try {
    ...
    try {
      ...
      const teiErrorsByFile = await handlePushResult(
        teiPushResults,
        metadataMapping
      );
    ...
    try {
      ...
      const enrErrorsByFile = await handlePushResult(
        enrPushResults,
        metadataMapping
      );
    ...
    try {
      ...
      const eventErrorsByFile = await handlePushResult(
        eventPushRetuls,
        metadataMapping
      );
    ...
    if (Object.keys(combinedErrorsByFile).length > 0) {
      const fileErrorMessages = Object.keys(combinedErrorsByFile).map(
        (fileName) => {
...
const getProgramMetadataNameMapping = ({
  programMetadata,
  ...
  const getTranslatedDisplayName = (item, locale) => {
    ...
    const formNameTranslation = item.translations.find(
      (t) => t.locale === locale && t.property === "FORM_NAME"
    );
    ...
    const nameTranslation = item.translations.find(
      ...
      program.programStages.forEach((stage) => {
        if (stage.dataElements) {
          stage.dataElements.forEach((dataElement) => {
            mapping[dataElement.id] = getTranslatedDisplayName(
              dataElement,
              locale
            );
          });
...
const PushToServerButton = () => {
  ...
  const { programMetadata, programMetadataMember } = useSelector(
